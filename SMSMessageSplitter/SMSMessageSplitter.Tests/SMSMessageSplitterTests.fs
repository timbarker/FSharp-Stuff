...
type ``A GSM Single part splitter``() = 
    ...
    [<Test>]
    member x.``Should handle escape characters``() = 
        let message = String.replicate 80 "€"
        let splits = SMSMessageSplitter.splitGSM message
        splits |> should haveLength 1
        splits |> should equal [ String.replicate 80 "€" ]
...
type ``A GSM multipart splitter``() = 
    ...
    [<Test>]
    member x.``Should handle escapte characters``() = 
        let message = String.replicate 77 "€"
        let splits = SMSMessageSplitter.splitGSM message
        splits |> should haveLength 2
        splits |> should equal [ String.replicate 76 "€"
                                 "€" ]
...
type ``A Unicode multi part splitter``() =
    ...
    [<Test>]
    member x.``Should not let last character be first part of surrogate pair``() =
...
type ``A GSM truncator``() =
    ...
    [<Test>]
    member x.``Should truncate extended chracters to correct length``() =
        let message = String.replicate 85 "€"
        let truncated = SMSMessageSplitter.truncateGSM message
        truncated |> should haveLength 80
        truncated |> should equal (String.replicate 80 "€") 

    [<Test>]
    member x.``Should respect escape characters at end of string``() =
...
type ``A Unicode truncator``() =
    ...
    [<Test>]
    member x.``Should repect surrogate pairs at end of string``() =
